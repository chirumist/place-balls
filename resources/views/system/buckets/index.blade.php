<x-guest-layout>
    <div class="flex justify-between mb-4">
        <h4 class="text-xl font-semibold">Buckets List</h4>
        <div class="flex gap-3">
            <x-secondary-link href="{{ route('welcome') }}">Home</x-secondary-link>
            <x-primary-link href="{{ route('buckets.create') }}">Create Buckets</x-primary-link>
        </div>
    </div>
                    ...
                    <tr class="bg-white">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex gap-2">
                                <a href="{{ route('buckets.edit', $value->id) }}"
                                ...
                                <div>
                                    <form id="deleteForm-{{ $value->id }}"
                                        action="{{ route('buckets.destroy', $value->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
